...
async def log_all_updates(handler, event, data):
    has_webapp = event.web_app_data is not None
    text_preview = event.text[:50] if event.text else "(немає тексту)"
    logger.info(f"[MIDDLEWARE] Від {event.from_user.id} | web_app_data: {has_webapp} | text: {text_preview}")
...
async def on_startup(bot: Bot):
    try:
        if WEBHOOK_URL:
            ...
            logger.warning("⚠️ WEBHOOK_URL не задано!")
    except Exception as e:
        logger.error(f"❌ Помилка встановлення webhook: {e}")

async def on_shutdown(bot: Bot):
        # НЕ видаляємо webhook — він повинен залишатися для Render
        try:
                await bot.session.close()
                logger.info("🛑 Сесія бота закрита")
        except Exception as e:
                logger.error(f"❌ Помилка закриття сесії: {e}")
