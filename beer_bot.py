...
async def health(request):
    return web.Response(text="🍺 Пивний Крафт бот працює! | Polling mode")

async def main():
        # Видаляємо старий webhook, щоб не було конфліктів
        await bot.delete_webhook(drop_pending_updates=True)
        logger.info("🤖 Бот запущено в режимі POLLING")
        
        # Створюємо aiohttp сервер для keep-alive (Render не вб'є процес)
        app = web.Application()
