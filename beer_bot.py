...
#  НАЛАШТУВАННЯ БОТА
# ═══════════════════════════════════════════════════════════════
API_TOKEN = '8872901197:AAFz5lyKIOpbMdwA70hxLoP5i1EU5r4Fn5s'          # ← Замініть на токен від @BotFather
CHAT_ID_MANAGERS = '4991707736'  # ← ID чату/групи менеджерів

# Посилання на Web App меню (розмістіть index.html на хостингу або GitHub Pages)
...
def format_cart(cart: dict) -> tuple[str, int]:
    ...
    for item_id, info in cart.items():
        ...
        if unit == "г":
            # Закуски: ціна за 50г, count у грамах
            qty_units = count / 50
            item_total = int(price * qty_units)
            qty_str = f"{int(count)} г"
        else:
            # Рідина: ціна за 0.5л, count у літрах
            qty_units = count / 0.5
            item_total = int(price * qty_units)
            qty_str = f"{count} л"
...
async def process_phone(message: types.Message, state: FSMContext):
    if message.contact:
        ...
        if not phone.replace("+", "").replace("-", "").replace(" ", "").isdigit():
            await message.answer("⚠️ Будь ласка, введіть коректний номер телефону або натисніть кнопку 'Поділитися номером'.")
            return
...
async def process_promo_and_finish(message: types.Message, state: FSMContext):
    ...
    try:
        ...
        logging.error(f"Помилка відправки менеджеру: {e}")
